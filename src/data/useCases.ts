...
export const useCases: UseCaseCard[] = [
  ...
  },
  {
    ...
    domain: 'Регуляторика / отчётность',
    deployment: 'Частный VNet',
    riskLevel: 'Низкий–средний',
    description:
      'AI ведёт календарь обязательных форм (Роснедра, ЦДУ ТЭК, ФНС, Ростехнадзор, Росприроднадзор, Росстат), через RAG вытягивает нужные цифры из 1С/АСУ ТП/ЛИМС, заполняет шаблоны и кладёт черновик менеджеру на проверку и отправку.',
    businessValue: 'Ноль пропущенных дедлайнов. Черновик формы за минуты, каждая цифра - со ссылкой на источник.',
    timeSaved: 'дни/форма · десятки форм в год',
    tools: ['LLM (on-prem)', 'RAG по методикам', 'Шаблонизатор форм', 'Календарь дедлайнов'],
    highlighted: true,
  },
  {
    id: 'UC-07',
    title: 'Проверка актов выполненных работ и счетов к оплате',
    domain: 'Финансы / контроль оплат',
    deployment: 'On-prem (ERP)',
    riskLevel: 'Средний',
    description:
      'Когнитивный кост-инженер сводит договор, факт выполнения (акт КС-2, справка КС-3) и платёжные документы (счёт, счёт-фактура/УПД) - сверяет расценки, объёмы, этапность освоения, лимиты, НДС и тайминг, собирает реестр к оплате. Вместо ручной проверки в SAP MM.',
    businessValue: 'Проверка инвойса минуты вместо дней. Завышенные объёмы/ставки и дубли ловятся до оплаты.',
    timeSaved: 'дни/счёт · контроль освоения договора',
    tools: ['OCR/RAG', '3-way match', 'Кост-контроль', '1С:ERP'],
    highlighted: true,
  },
  {
    id: 'UC-08',
